...
### Part 1
In the next block the objects of the charts are instantiated and set to represent a bivariate distribution in case of the contingency table and the scatterplot. Instead, the histogram represent a univariate distribution.
{% highlight C# %}
...
As last there's the code of the DrawScene function of the histogram that draws it.
{% highlight C# %}
public void DrawScene()
{
    g.Clear(pb.BackColor);
